      ...
      <div className="shell py-20 sm:py-28 lg:py-36">
        <div className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr] lg:items-end lg:gap-20">
          <div>
            ...
            <p className="mt-6 max-w-md text-base leading-relaxed text-[#4a4a4a] sm:text-lg">
            ...
            <Link
              href="/dealers"
              className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[#0a0a0a] transition hover:text-volt"
            >
              Explore our network
          ...
          <div className="rounded-[1.75rem] border border-black/[0.10] bg-[#f7f7f7] p-5 shadow-[0_30px_90px_-35px_rgba(0,0,0,0.14)] sm:p-8 lg:p-10">
            {partners.length > 0 ? (
              <div className="grid grid-cols-2 divide-x divide-y divide-black/[0.10] border-l border-t border-black/[0.10] sm:grid-cols-3">
                {partners.map((dealer) => (
                  <Link
                    key={dealer.id}
                    href={`/dealers/${dealer.slug}`}
                    aria-label={`View ${dealer.name}`}
                    className="group flex min-h-28 items-center justify-center border-r border-b border-black/[0.10] p-6 transition hover:bg-black/[0.025] sm:min-h-36 sm:p-8"
                  >
